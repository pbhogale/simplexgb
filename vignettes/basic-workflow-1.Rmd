...
library(simplexgb)
data(wisccancerdata)
train_struct <- prepare_training_set(df = wisc_cancer_data, target_variable = 'diagnosis')
```
...
train_struct <- prepare_training_set(train_df, target_variable = 'diagnosis')
hyp <- guess_hyperparameters(train_structure = train_struct)
...
```{r}
pred_df <- get_predictions_xgb(xgbmodel, test_df = test_df)
pred_df %>% head()
...
```{r}
model_struct <- train_linear_model(train_struct, xgbmodel, hyp)
plot(model_struct[["models"]][["linear_model"]])
```

ˆ
```{r}
pred_df_linear <- get_predictions_linear(model_struct, test_df = test_df)
pred_df_linear %>% head()
test_df_preds <- test_df %>% 
...
preds_rf <- get_predictions_rf(model_structure = model_struct, test_df = test_df)
test_df_preds <- test_df %>% 
